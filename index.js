...
function getUsersFromDatabase(array) {
  let userArr = Array.from(array, users => {
    return {
      id: users.id,
...
function findElementIndexes(arr, element) {
  if (Array.isArray(arr)) {
    ...
    let newArr = [];
    if (arr.includes(firstElem) && lastElem !== firstElem) {
...
function copyAndSwapElements(arr, target, start, end) {
  if (target < arr.length && end <= arr.length) {
...
function customAt(arr, index) {
  if (Array.isArray(arr) && index <= arr.length) {
    if (typeof arr.at(index) === 'string') {
      return `${arr.at(index)} Елемент є рядком`;
    } else if (typeof arr.at(index) === 'number') {
      return `${arr.at(index)} Елемент є числом`;
    } else if (typeof arr.at(index) === 'object')
      return `${arr.at(index)} Елемент є обєктом`;
  } else {
    return undefined;
  }
...
function customIncludes(arr, element) {
  if (Array.isArray(arr)) {
    let checkElement = arr.includes(element);
    let times = arr.filter(elem => elem === element).length;
    return `${checkElement}, ${times}`;
  } else {
    return undefined;
  }
  // Перевіряємо, чи вхідний параметр є масивом
